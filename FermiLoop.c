...
int main(int argc, char *argv[])
{
  ...
  if (fo_inp == 1){
    ...
    /* Added by N. Yamaguchi ***/
    input_logical("Eigen.Newton",&switch_Eigen_Newton,0);
  ...
  if ((fo_inp == 1) && (fo_wf == 1)){
	...
	l_max=calcBandMax= l_max<calcBandMax ? l_max: calcBandMax;
	l_min=calcBandMin= l_min>calcBandMin ? l_min: calcBandMin;
	l_cal=l_max-l_min+1;
      }
    }
    /* ***/
	...
	if ((E1-EF)*(E2-EF) <=0){
      ...
      if (calcBandbyband) {
	if (calcBandMax<l_min || calcBandMin>l_max) {
	  PRINTFF("Error: No bands are found in the range of %d-%d.", calcBandMin, calcBandMax);
	  MPI_Finalize();
	  return 0;
	} else {
	  l_max=calcBandMax= l_max<calcBandMax ? l_max: calcBandMax;
