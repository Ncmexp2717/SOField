...
void Input( FILE *fp, int Print_datFile){
  ...
  ****************************************************/
  ...
  sprintf(makeinp,"input.dat");
  if (Print_datFile == 1) fp_makeinp = fopen(makeinp,"w");
  ...
  for (i=1; i<=num_lines; i++){
    ...
    if (Print_datFile == 1) fprintf(fp_makeinp,"%s",strg);
  }

  if (Print_datFile == 1) fclose(fp_makeinp);
}
